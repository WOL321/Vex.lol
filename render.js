...
function buildPublicHTML(c, opts) {
  ...
  const socialsHTML = (c.socials || []).map(s => {
    ...
    return `<a class="social" href="${esc(s.url)}" target="_blank" rel="noopener" title="${esc(s.label || s.platform)}">${iconHTML}</a>`;
  }).join('');
...
.card{width:100%;max-width:var(--card-w);background:rgba(19,20,32,0.55);background-image:linear-gradient(160deg,color-mix(in srgb,var(--accent1) 16%,transparent),transparent 45%,color-mix(in srgb,var(--accent2) 10%,transparent)),radial-gradient(ellipse 80% 50% at 50% 0%,color-mix(in srgb,var(--accent1) 20%,transparent),transparent 70%);border:1px solid rgba(255,255,255,0.09);border-radius:var(--radius);backdrop-filter:blur(26px) saturate(160%);-webkit-backdrop-filter:blur(26px) saturate(160%);padding:36px 30px 26px;box-shadow:0 32px 90px -30px rgba(0,0,0,0.9),0 0 0 1px rgba(255,255,255,0.04),0 0 calc(70px * var(--glow)) -10px color-mix(in srgb,var(--accent1) 50%,transparent);position:relative;overflow:hidden;}
