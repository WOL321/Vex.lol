...
const ICONS = {
  twitter: '<path d="M4 4l7.5 9.6L4.5 20H7l5.2-5.8L16.5 20H20l-8-10.2L18.8 4H16l-4.7 5.3L7.5 4H4z" fill="currentColor" stroke="none"/>',
  github: '<path d="M12 2a10 10 0 00-3.2 19.5c.5.1.7-.2.7-.5v-1.8c-2.8.6-3.4-1.3-3.4-1.3-.5-1.1-1.1-1.5-1.1-1.5-.9-.6.1-.6.1-.6 1 .1 1.5 1 1.5 1 .9 1.5 2.3 1.1 2.9.8.1-.7.4-1.1.6-1.4-2.2-.3-4.6-1.1-4.6-5 0-1.1.4-2 1-2.7-.1-.3-.5-1.3.1-2.7 0 0 .8-.3 2.8 1a9.6 9.6 0 015 0c1.9-1.3 2.8-1 2.8-1 .6 1.4.2 2.4.1 2.7.6.7 1 1.6 1 2.7 0 3.9-2.4 4.7-4.6 5 .4.3.7.9.7 1.9v2.8c0 .3.2.6.7.5A10 10 0 0012 2z" fill="currentColor" stroke="none"/>',
  discord: '<path d="M17.5 5.5a15 15 0 00-3.6-1.1l-.2.4c1.3.3 2.5.8 3.6 1.5a13 13 0 00-10.6 0c1.1-.7 2.4-1.2 3.7-1.5l-.2-.4a15 15 0 00-3.7 1.1C4 8.7 3.4 11.8 3.6 14.9a13 13 0 004 2l.7-1.1a8 8 0 01-1.4-.7l.3-.3a10 10 0 008.6 0l.3.3c-.4.3-.9.5-1.4.7l.7 1.1a13 13 0 004-2c.3-3.6-.6-6.7-2.9-9.4zM9.6 13.2c-.7 0-1.3-.7-1.3-1.5s.6-1.5 1.3-1.5 1.3.7 1.3 1.5-.6 1.5-1.3 1.5zm4.8 0c-.7 0-1.3-.7-1.3-1.5s.6-1.5 1.3-1.5 1.3.7 1.3 1.5-.6 1.5-1.3 1.5z" fill="currentColor" stroke="none"/>',
  instagram: '<rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.4" cy="6.6" r="1"/>',
  youtube: '<rect x="2.5" y="5.5" width="19" height="13" rx="4"/><path d="M10.5 9.5l5 2.5-5 2.5v-5z" fill="currentColor" stroke="none"/>',
  tiktok: '<path d="M14 3v10.5a2.8 2.8 0 11-2.3-2.75V8.3A5.3 5.3 0 1017 13.6V9.9a6.6 6.6 0 003.5 1V7.4A4 4 0 0117 3h-3z" fill="currentColor" stroke="none"/>',
  spotify: '<circle cx="12" cy="12" r="9"/><path d="M7 10.3c3-1 6.8-.6 9.3.9M7.3 13.2c2.5-.7 5.5-.4 7.7.8M7.6 16c2-.5 4.4-.3 6.1.6" stroke-linecap="round"/>',
  twitch: '<path d="M5 3l-1 4v11h4v3l3-3h3l4-4V3H5zm10 10l-2 2h-3l-2 2v-2H6V5h11v8z"/><path d="M14 8v3M10.5 8v3"/>',
  telegram: '<path d="M21 4L3 11l6 2m12-9L9 13m12-9l-4 16-6-4m0 0l-2 2v-4"/>',
...
:root {
  --void: #0a0a10;
  --panel: rgba(19,20,32,${(c.theme.glass / 100).toFixed(2)});
  --panel-solid: #131420;
  --border: rgba(255,255,255,0.08);
  --border-hi: rgba(255,255,255,0.15);
  --accent1: ${c.theme.accent1};
  --accent2: ${c.theme.accent2};
  --text: #eceef5;
  --text-dim: #8a8ea3;
  --text-faint: #4b4e63;
  --success: #7fd8a6;
  --radius: ${c.theme.radius}px;
  --card-w: ${c.theme.cardWidth}px;
...
.bg-grid{position:fixed;inset:0;z-index:0;background-color:var(--void);
...
.entrance-name{font-family:var(--font-display);font-weight:700;font-size:clamp(38px,9vw,84px);letter-spacing:-0.02em;line-height:1;
  background:linear-gradient(120deg,#fff 20%,var(--accent1) 55%,var(--accent2) 100%);-webkit-background-clip:text;background-clip:text;color:transparent;opacity:0;animation:fadeUp .9s ease .35s forwards;}
...
.card{
  ...
  background-image:
    linear-gradient(160deg, color-mix(in srgb, var(--accent1) 16%, transparent), transparent 45%, color-mix(in srgb, var(--accent2) 10%, transparent)),
    radial-gradient(ellipse 80% 50% at 50% 0%, color-mix(in srgb, var(--accent1) 20%, transparent), transparent 70%);
  ...
  box-shadow:
    ...
    0 0 calc(70px * var(--glow)) -10px color-mix(in srgb, var(--accent1) 50%, transparent);
  position:relative;
  overflow:hidden;
}
.card::before{
  ...
  background:linear-gradient(145deg, color-mix(in srgb, var(--accent1) 45%, transparent), transparent 40%, color-mix(in srgb, var(--accent2) 30%, transparent));
...
.social:hover{border-color:var(--border-hi);background:${c.theme.accent1}1a;box-shadow:0 0 calc(18px * var(--glow)) -3px var(--accent1);}
...
(function(){
  ...
  if(['particles','snow','rain','fireflies'].indexOf(BG_STYLE) !== -1){
    ...
    if(canvas){
      ...
      var COUNT = BG_STYLE === 'rain' ? 140 : 95;
      ...
      for(var i=0;i<COUNT;i++){
        ...
        else particles.push({x:Math.random()*innerWidth,y:Math.random()*innerHeight,vx:(Math.random()-.5)*.3,vy:(Math.random()-.5)*.3,r:Math.random()*1.8+.7});
      }
      ...
      function drawFrame(){
        ...
        for(var i=0;i<particles.length;i++){
          ...
          if(BG_STYLE==='particles' && REACTION !== 'none'){
            ...
            if(d < 130){
              var f = (130-d)/130;
              var sign = REACTION === 'attract' ? -1 : 1;
              p.vx += sign * (dx/d) * f * 0.9;
              p.vy += sign * (dy/d) * f * 0.9;
            }
          }
          p.vx *= 0.98; p.vy *= 0.98;
          ...
          else { ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle = BG_STYLE==='snow' ? 'rgba(255,255,255,0.7)' : 'rgba('+rgb[0]+','+rgb[1]+','+rgb[2]+',0.6)'; ctx.fill(); }
        }
        if(BG_STYLE==='particles'){
          for(var i=0;i<particles.length;i++){
            for(var j=i+1;j<particles.length;j++){
              var a=particles[i], b=particles[j], dx=a.x-b.x, dy=a.y-b.y, d=Math.sqrt(dx*dx+dy*dy);
              if(d<130){ ctx.strokeStyle='rgba('+rgb[0]+','+rgb[1]+','+rgb[2]+','+(0.13*(1-d/130))+')'; ctx.lineWidth=.6; ctx.beginPath(); ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); ctx.stroke(); }
            }
          }
        }
        t++;
  ...
  function loadTrack(i, autoplay){
    ...
    if(audioEl){
      if(tr.url && !tr.url.includes('youtube.com') && !tr.url.includes('youtu.be')){
        audioEl.src = tr.url;
      } else {
  ...
  if(npTitle){
    ...
    if(npProgress) npProgress.addEventListener('click', function(e){
      ...
      var rect = npProgress.getBoundingClientRect();
