...
const db = {
  ...
  setProfile(userId, config) {
    return withLock(async () => {
      ...
      state.profilesByUserId[userId] = {
        ...
        clicks: existing && existing.clicks ? existing.clicks : {},
  ...
  incrementClick(userId, platform) {
    return withLock(async () => {
      const p = state.profilesByUserId[userId];
      if (!p) return 0;
      if (!p.clicks) p.clicks = {};
      const key = String(platform || 'link').toLowerCase().slice(0, 40);
      p.clicks[key] = (p.clicks[key] || 0) + 1;
      await persist();
      return p.clicks[key];
    });
  }
